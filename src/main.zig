...
export fn kmain(magic: u32, info: u32) callconv(.c) void {
    ...
    // M17: slab allocator + ring buffer + libk extras.
    {
        ...
        const libk = @import("libk.zig");
        var ok17 = true;
        // slab: alloc 100 objects, free, reuse
        var sl = slab.Slab.init(heap.allocator(), 24, 8, 16);
        defer sl.deinit();
        var ptrs: [100][*]u8 = undefined;
        for (&ptrs) |*pp| { pp.* = sl.alloc() orelse { ok17 = false; break; }; }
        if (sl.stats().live != 100) ok17 = false;
        for (ptrs) |pp| sl.free(pp);
        if (sl.stats().live != 0) ok17 = false;
        // ring
        var rb = ring.Ring(8){};
        if (!rb.push('a') or !rb.push('b')) ok17 = false;
        if (rb.pop() != 'a' or rb.pop() != 'b') ok17 = false;
        // libk extras
        if (libk.strncmp("abc", "abd", 2) != 0) ok17 = false;
        if (libk.indexOf("hello", 'l') != 2) ok17 = false;
        if (libk.toUpper('a') != 'A' or libk.toLower('Z') != 'z') ok17 = false;
        var fb: [24]u8 = undefined;
        if (!std.mem.eql(u8, libk.formatDec(&fb, 12345), "12345")) ok17 = false;
